      ...
      {Object.entries(items).map(([key, content]) => (
        <AccordionPrimitive.Item key={key} value={key}>
          <AccordionPrimitive.Header>
            <AccordionPrimitive.Trigger className="group w-full flex items-center justify-between px-4 py-2 outline-none focus:outline-none focus:ring-2 focus:ring-emerald-300">
              <span>{key}</span>
              ...
              {currentKeys.includes(key as ItemKey) && (
                <AccordionPrimitive.Content asChild forceMount>
                  <motion.div
                    ...
                    exit="closed"
                  >
                    <div className="p-4 mt-1 bg-slate-100">{content}</div>
                  </motion.div>
